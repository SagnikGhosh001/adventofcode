...
const intcode = (copyArray, ip = 0, rb = 0, input) => {
  ...
  while (ip < copyArray.length) {
    ...
    if (opcode === 4) {
      return { ip, rb, output: result.output, halt: false };
    }
...
const adjacentOffsets = [[1, 0], [0, 1], [-1, 0], [0, -1]];

const findAdjacent = (coordinate, visited) => {
  const adjacent = [];
  for (const offset of adjacentOffsets) {
    const newCord = [coordinate[0] + offset[0], coordinate[1] + offset[1]];
    const key = `${newCord[0]},${newCord[1]}`;

    if (allPath.has(key) && !visited.has(key)) {
      visited.add(key);
      adjacent.push(newCord);
    }
  }
...
const calculateTime = (oxygenLoc) => {
  let time = 0;

  const visited = new Set([`${oxygenLoc[0]},${oxygenLoc[1]}`]);
  let queue = [oxygenLoc];
  while (queue.length > 0) {
    const nextQueue = [];

    for (const coord of queue) {
      nextQueue.push(...findAdjacent(coord, visited));
    }

    if (nextQueue.length > 0) time++;
    queue = nextQueue;
  }
  return time;
};

let allPath = new Set(["0,0"]);
...
const main = (
  ...
  if (output === 2) {
    if (path.length === 0 || currentPath.length < path.length) {
      path = [...currentPath];
    }
  ...
  for (const input of DIRECTIONS) {
    ...
    const { ip, rb, output } = intcode(newMemory, instPoint, relative, input);
    const newCoord = move(input.toString(), currentPath.at(-1));

    const key = newCoord.join(",");
    if (output === 0 || visited.has(key)) continue;

    allPath.add(key);
    visited.add(key);
    currentPath.push(newCoord);

    main(newMemory, ip, rb, output, currentPath, visited);
...
main([...memory], 0, 0, 1);
console.log(path.length - 1, "length");
const minutes = calculateTime(path.at(-1));
console.log(minutes);
