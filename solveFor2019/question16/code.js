...
const main = () => {
  const largeInput = input.repeat(1);
  const messageOffset = +input.slice(0, 7);
  let inputArray = [...largeInput].map(Number);
  ...
  // console.log(messageOffset, inputArray.length, inputArray);
  console.log(inputArray.slice(0, 0 + 8));
};
