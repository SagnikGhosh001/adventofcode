...
const wiresUpdatedLoc = (x, y, inst) => {
  const offset = +inst.slice(1);
  const heading = inst[0];
  const path = [];

  for (let ele = 0; ele < offset; ele++) {
    x = directions(x, y, heading)[0];
    y = directions(x, y, heading)[1];
    path.push([x, y]);
  }

  return { path, x: path.at(-1)[0], y: path.at(-1)[1] };
};
...
const run = () => {
  ...
  for (let index = 0; index < firstWire.inst.length; index++) {
    const updatedValues = wiresUpdatedLoc(
      firstWire.x,
      firstWire.y,
      firstWire.inst[index],
    );
    const updatedX = updatedValues.x;
    const updatedY = updatedValues.y;

    firstPath = firstPath.concat(updatedValues.path);

    firstWire.x = updatedX;
    firstWire.y = updatedY;
  }

  for (let index = 0; index < secondWire.inst.length; index++) {
    const updatedValues = wiresUpdatedLoc(
      secondWire.x,
      secondWire.y,
    ...
    secondPath = secondPath.concat(updatedValues.path);

    secondWire.x = updatedX;
    secondWire.y = updatedY;
  }
  let firstStep = 0;
  let secondStep = 0;
  ...
  for (let i = 0; i < firstPath.length; i++) {
    ...
    for (let j = 0; j < secondPath.length; j++) {
      if (
        firstPath[i][0] === secondPath[j][0] &&
        firstPath[i][1] === secondPath[j][1]
      ) {
        const distance = Math.abs(0 - firstPath[i][0]) +
          Math.abs(0 - firstPath[i][1]);
        minDistance = Math.min(minDistance, distance);
        minStep = Math.min(minStep, firstStep + secondStep);
      }
      secondStep++;
    }
    firstStep++;
